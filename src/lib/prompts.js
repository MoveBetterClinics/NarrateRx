...
export const VOICE_MODES = [
  {
    ...
    emoji: '🏥',
    description: `Speaking for the clinic. The interview is about how ${workspace.name} as a team approaches this — outputs use "we" and "our team."`,
  },
  {
    id: 'personal',
...
Brand attribution still applies: end the piece with a signature line on its own — "— ${clinicianName}, ${workspace.name}, ${workspace.location}". Internal links, paradigm vocabulary, and links to ${workspace.name} resources should still appear naturally.`
  }
  // Practice voice — current behavior, with explicit conversion guidance.
  const clinicianMention = assetType === 'video'
    ? `${clinicianName} is the on-camera clinician and expert, but the brand being promoted is ${workspace.name}. Scripts should introduce ${clinicianName} as "our clinician" or "part of the ${workspace.name} team." All CTAs, bookings, and references point to ${workspace.name}, not to ${clinicianName} personally.`
...
This content is branded for ${workspace.name} as a clinic — NOT for the individual clinician. The subject is always "we at ${workspace.name}" or "our team" or "our approach." Even if the clinician used "I" or "me" in the interview, convert it to clinic voice in the output (e.g., "I see this in patients" → "We see this in patients at ${workspace.name}"). ${clinicianMention}`
}
...
PRIOR COVERAGE — ${condition} has already been interviewed at ${workspace.name}:
${formatted}
...
`
  ...
  return `You are a content facilitator helping ${clinicianName} at ${workspace.name} think out loud about how they treat ${condition}. Your job is to pull out their clinical perspective efficiently so it can be turned into patient-facing content branded for ${workspace.name} as a whole.
${formatPNWContextForPrompt(condition)}${pastContext}
${workspace.name} context: ${workspace.prompt.clinicContext}
...
export function getBlogPostSystemPrompt(clinicianName, condition, tone = 'smart', voiceMode = 'practice', prototypeId = null) {
  ...
  return `You are a content writer for ${workspace.name} in ${workspace.location}. Based on the interview transcript below with ${clinicianName} about treating ${condition}, write an engaging, on-brand blog post targeted at ${workspace.region} readers.

${getFramingRule({ voiceMode, clinicianName, assetType: 'blog' })}

${workspace.name.toUpperCase()} BRAND VOICE:
...
${workspace.prompt.internalLinksMarkdown}
...
- The CTA section must always link to ${workspace.prompt.bookingUrl} for booking

BLOG POST FORMAT (write in Markdown):

# [Headline: compelling, specific, hopeful — about the condition${isPersonal ? '' : ` and ${workspace.name}'s approach`}. Never include the clinician's name in the headline.]
...
[Explain the condition in plain language from a clinical perspective — what's actually happening in the body and why standard approaches often fall short. Include 1–2 links here: one internal to a related ${workspace.name} post, one external to an authoritative source.]

## ${isPersonal ? `My Approach to ${condition}` : `The ${workspace.name} Approach to ${condition}`}
[${isPersonal
  ...
  : `${workspace.name}'s specific treatment approach — what makes it different, what the process looks like. Use "we" and "our team." Make it concrete and specific to what was shared in the interview.`}${workspace.prompt.signatureSystemName ? ` Link to ${workspace.prompt.signatureSystemName} (${workspace.prompt.signatureSystemUrl}) if relevant.` : ''}]

## ${isPersonal ? 'What I See in My Patients' : 'What Our Patients Experience'}
[${isPersonal
  ? `Walk through the patient journey from first visit onward in first person — what I do, what changes, realistic timeline. Cite any success stories from the interview (anonymized). Link to a relevant ${workspace.name} post if it fits naturally.`
  : `Walk through the patient journey from first visit onward — what happens, what changes, realistic timeline. Cite any success stories from the interview (anonymized). Use "our patients" language. Link to a relevant ${workspace.name} post if it fits naturally.`}]
...
[${isPersonal
  ...
  : `The key clinical observation from the interview — framed as ${workspace.name}'s team perspective. This makes the post human and builds trust. The clinician's name may appear here once naturally if it adds credibility, e.g. "As our clinician ${clinicianName} puts it…"`}]

## Ready to Move Better?
[Warm, encouraging CTA — 3–4 sentences. Reinforce movement as the solution. Invite them to book at [${workspace.name}](${workspace.prompt.bookingUrl}). Keep it conversational, not salesy.]
${isPersonal ? '' : `
---
...
export function getSocialBatchSystemPrompt(clinicianName, condition, campaignContext = '', tone = 'smart', voiceMode = 'practice', prototypeId = null) {
  ...
  return `Based on the blog post provided, generate social media content for ${workspace.name}. The post is about ${condition}.

${getFramingRule({ voiceMode, clinicianName, assetType: 'social' })}
${patientContext ? `\n${patientContext}\n` : ''}
${workspace.name}'s audience: ${workspace.prompt.audienceDescription}
...
- Share the single most compelling insight from the blog ${isPersonal ? 'in my own first-person voice' : `as ${workspace.name}'s team perspective`}
...
- Skip a line, then add 8–10 hashtags: condition-specific, movement, ${workspace.prompt.locationKeyword}/${workspace.regionShort}, and brand tags

---FACEBOOK---
- 100–150 words
${isPersonal ? `- Written in my own first-person voice — sharing with the local ${workspace.prompt.locationKeyword} community from a personal angle` : `- Written as ${workspace.name} the clinic sharing with the local ${workspace.prompt.locationKeyword} community`}
...
- Share ${isPersonal ? 'my key perspective' : `${workspace.name}'s key perspective`} — what makes the approach different
...
- Close with: "Book your assessment at ${workspace.name} — link in profile"
...
- Include what my approach gets right that others miss`
  : `- Written from ${workspace.name}'s company voice — for other clinicians, coaches, employers, and referring providers
- Frame as clinical perspective: "At ${workspace.name}, we approach ${condition} differently…"
- Include what ${workspace.name}'s approach gets right that others miss
...
PIN TITLE: (max 100 characters, include keywords naturally — brand as ${workspace.name})
PIN DESCRIPTION: (200–400 characters, keyword-rich natural language, include ${workspace.website})
BOARD: (${workspace.prompt.pinterestBoards})${campaignContext}
${getToneModifier(tone)}`
}
...
export function getVideoScriptBatchSystemPrompt(clinicianName, condition, campaignContext = '', tone = 'smart', voiceMode = 'practice', prototypeId = null) {
  ...
  const isPersonal = voiceMode === 'personal'
  const patientContext = getPatientContextForPrompt(prototypeId)
  return `Based on the blog post provided, write two video scripts for ${workspace.name} about ${condition}.

${getFramingRule({ voiceMode, clinicianName, assetType: 'video' })}
${patientContext ? `\n${patientContext}\n` : ''}
${workspace.name}'s audience: ${workspace.prompt.audienceShort}
...
[INTRO — 30 seconds]
${firstName} introduces themselves naturally: name, role at ${workspace.name}, one sentence on ${isPersonal ? 'their movement-first philosophy' : `${workspace.name}'s movement-first philosophy`}.

[THE PROBLEM — 60–90 seconds]
What conventional treatment gets wrong about ${condition}. Specific, not generic. Framed as ${isPersonal ? `${firstName}'s clinical perspective in first person` : `${workspace.name}'s clinical perspective`}.

[${isPersonal ? 'MY APPROACH' : `THE ${workspace.name.toUpperCase()} APPROACH`} — 2–3 minutes]
${isPersonal
  ...
  : `${workspace.name}'s actual assessment and treatment process for ${condition}. Patient-friendly language. Use "we" and "our team." Add [B-ROLL: ...] notes in brackets where relevant footage would help.`}

[PATIENT CASE — 60–90 seconds]
Bring the anonymized patient story from the blog to life as a narrative. What changed, how fast, what they can do now. ${isPersonal ? `Reference it as one of ${firstName}'s patients, in first person.` : `Reference it as a ${workspace.name} patient.`}
...
[CTA — 30 seconds]
Warm, direct close. Invite viewers to book a movement assessment at ${workspace.name} in ${workspace.prompt.locationKeyword}. Say the URL naturally: "You can book at ${workspace.prompt.spokenUrl}" and tell viewers to check the description for the link.
...
- 5–8 keyword hashtags for YouTube (#${condition.replace(/\s+/g, '')} ${workspace.prompt.locationHashtag} ${workspace.prompt.brandHashtag} etc.)
...
[BODY — 30–40 seconds]
3–4 short punchy points from ${isPersonal ? `${firstName}'s clinical perspective in first person` : `${workspace.name}'s clinical perspective`}. 1–2 sentences each. Plain language, no jargon. Add [ON SCREEN TEXT: ...] for any text overlays.

[CLOSE — 10 seconds]
Soft CTA: "If you're dealing with ${condition} in ${workspace.prompt.locationKeyword}, follow for more — link in bio to book at ${workspace.name}."

CAPTION:
50–80 word TikTok caption with 5–6 relevant hashtags. Brand as ${workspace.name}.${campaignContext}
${getToneModifier(tone)}`
}
...
export function getMarketingBatchSystemPrompt(clinicianName, condition, campaignContext = '', tone = 'smart', prototypeId = null) {
  ...
  const patientContext = getPatientContextForPrompt(prototypeId)
  return `Based on the blog post provided, generate three marketing assets for ${workspace.name} about ${condition}. Use the blog post as your source of truth.
${patientContext ? `\n${patientContext}\n` : ''}
CRITICAL FRAMING RULE:
All assets are branded for ${workspace.name} as a clinic. The clinician's expertise informs the content but ${workspace.name} is always the subject. Use "we," "our team," and "${workspace.name}" throughout. The clinician's name (${firstName}) may appear once in the email as a credibility signal but should not appear in headlines, page titles, or ad copy.

Output each section separated by the exact markers below.

---EMAIL NEWSLETTER---
Monthly patient newsletter for TrustDrivenCare delivery. Output MUST use the exact section markers below — each maps to a named field in the ${workspace.newsletterTemplateName} template.

---SUBJECT LINE---
One subject line. Curiosity- or benefit-driven. About the condition and what ${workspace.name} does — not the clinician. Under 50 characters.
...
---BODY PARAGRAPH 2---
${workspace.name}'s perspective. 3–5 sentences on what makes the approach different for ${condition}. Use "we" and "our team." May reference "one of our clinicians, ${firstName}" once for credibility.

---BODY PARAGRAPH 3---
Patient story + bridge to action. 3–4 sentences. Anonymized case from the blog if available, framed as a ${workspace.name} patient. End with a natural transition toward booking.

---CTA TEXT---
Button label only. 4–7 words. Action-oriented. E.g. "Book Your Movement Assessment" or "Start Moving Better Today".

---CTA URL---
${workspace.prompt.bookingUrl}
...
---LANDING PAGE---
Conversion-focused landing page copy for a condition-specific ${workspace.name} page about ${condition}.

HEADLINE: (compelling, specific, hopeful — under 10 words — about the condition and ${workspace.name}. No clinician name.)
SUBHEADLINE: (one sentence expanding — what ${workspace.name} offers for ${condition})
...
H2 + 3–4 sentences on ${workspace.name}'s specific assessment and treatment process. Use "we" and "our team."${workspace.prompt.signatureSystemName ? ` Reference ${workspace.prompt.signatureSystemName} if it fits, linking to ${workspace.prompt.signatureSystemUrl}.` : ''}

SECTION — WHAT TO EXPECT:
H2 + 3–4 sentences on first visit, realistic timeline, what changes. Always "our patients" language.

SECTION — PATIENT STORY:
H2 + 3–4 sentences. Anonymized, specific, outcomes-focused. Frame as a ${workspace.name} patient.

TRUST SIGNALS:
4–6 one-line bullet points (e.g. "Movement-first — we treat root causes, not just symptoms")

CLOSING CTA:
H2 + 2 sentences + button text. Link destination: ${workspace.prompt.bookingUrl}

SEO:
TITLE TAG: (under 60 characters, include "${condition}" and "${workspace.prompt.locationKeyword}" and "${workspace.name}")
META DESCRIPTION: (under 160 characters, compelling, includes condition and location)

---GOOGLE ADS---
Google Responsive Search Ad copy for ${workspace.name} targeting ${condition} searches in ${workspace.prompt.locationKeyword}.
...
DISPLAY PATH: ${workspace.websiteHostname}/${conditionSlug}
...
Mix brand terms (${workspace.name}, ${workspace.prompt.locationKeyword}), condition terms (${condition}), and benefit terms (pain relief, root cause, movement assessment). Avoid superlatives unless substantiated. No prices.

---INSTAGRAM ADS---
Meta Ads creative copy for ${workspace.name}, targeting ${condition} on Instagram in ${workspace.prompt.locationKeyword}. This will be pasted into Meta Ads Manager — output each field on its own line with the exact label shown.

PRIMARY TEXT: (125 chars recommended for above-the-fold, 2200 max — hook in the first sentence, lead with the problem or a counterintuitive insight about ${condition}, frame as ${workspace.name}'s perspective using "we"/"our team", end with a clear next step. No URLs in the body.)
[primary text]
...
- Recommend a real photo of ${workspace.name} clinicians or patients in motion over stock${campaignContext}
${getToneModifier(tone)}`
}
